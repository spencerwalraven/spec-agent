...
function formatJob(r) {
  return {
    ...
    lotSize:               r.lot_size                  || '',
    workAreas:             r.work_areas                || '',
    soilConditions:        r.soil_conditions           || '',
    accessIssues:          r.access_issues             || '',
    existingConditions:    r.existing_conditions       || '',
...
async function updateJobField(id, field, value) {
  const allowed = ['notes', 'status', 'priority', 'estimated_value', 'actual_value',
    ...
    'site_visit_date', 'lot_size', 'work_areas', 'soil_conditions', 'access_issues',
    'existing_conditions', 'client_preferences', 'client_budget', 'selected_tier',
    'estimate_link', 'proposal_link', 'contract_link', 'kickoff_link',
    'email_thread_id', 'proposal_sent_at', 'contract_signed_at',
